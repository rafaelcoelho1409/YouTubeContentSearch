...
#------------------------------------------------
###NODES
def search_youtube_videos(state: State):
    ...
    print(youtube_search_query)
    search_query = [youtube_search_query.search_query]
    search_results_dict = {}
    model_config = requests.get("http://fastapi:8000/model_config")
    if model_config.search_type == "Search":
        ...
        for query in stqdm.stqdm(search_query, desc = "Searching YouTube videos"):
            search_results = Search(
                query,
                filters = search_filters_dict).videos[:model_config.max_results]
        ...
        search_results_dict[model_config.video_url] = {
        ...
        search_results_dict[model_config.channel_url] = pd.DataFrame({
        ...
        search_results_dict[model_config.playlist_url] = pd.DataFrame({
...
def set_knowledge_graph(state: State):
    messages = state["messages"]
    streamlit_actions = state["streamlit_actions"]
    ...
    for document in stqdm.stqdm(documents, desc = "Transforming documents to graphs"):
        graph_documents += model_config.llm_transformer.convert_to_graph_documents([document])
...
def final_step(state: State):
    ...
    question_answer = requests.post(
        "http://fastapi:8000/youtube_content_search/answer",
        json = {"question": user_input}
    )
    #question_answer = rag_chain.invoke({"question": user_input})
    messages += [
        (
            "assistant",
            question_answer
...
def load_model():
    ...
    global neo4j_graph, vector_index, llm_transformer, workflow, graph
    ...
    ###NODES
    workflow.add_node("search_youtube_videos", search_youtube_videos)
    workflow.add_node("set_knowledge_graph", set_knowledge_graph)
...
def build_youtube_search_agent(user_input: str):
...
def build_entity_chain(question: str):
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
...
def build_rag_chain(user_input: str):
    ###TOOLS
...
def get_context_to_search(request: SearchRequest):
    events = stream_graph_updates(request.context_to_search)
    return events
